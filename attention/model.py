...
class StructuredSelfAttention(BasicModule):
    ...
    def forward(self,x):
        ...
        outputs, hidden_state = self.lstm(embeddings, hidden_state)#step1 get LSTM outputs

        selfatt = torch.tanh(self.linear_first(outputs))
        selfatt = self.linear_second(selfatt)
        selfatt = F.softmax(selfatt, dim=1)
        selfatt= selfatt.transpose(1, 2)
        self_att = torch.bmm(selfatt, outputs)   #step2 get self-attention
        ...
        label_att= torch.cat((torch.bmm(m1,h1),torch.bmm(m2,h2)),2)#step3 get label-attention
        # label_att = F.normalize(label_att, p=2, dim=-1)
        # self_att = F.normalize(self_att, p=2, dim=-1)
